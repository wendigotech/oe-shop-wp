<div <?php if(empty($_GET['context']) || $_GET['context'] !== 'edit') echo get_block_wrapper_attributes( array('class' => "col-md col-sm-6 py-3", ) ); else echo 'data-wp-block-props="true"'; ?>>
    <div class="position-relative text-dark">
        ...
        <div class="bottom-0 end-0  pb-5 pe-4 position-absolute ps-4 start-0">
            <h2 class="h5 mb-3"><?php echo PG_Blocks_v3::getAttribute( $args, 'title' ) ?></h2>
            <a href="<?php echo (!empty($_GET['context']) && $_GET['context'] === 'edit') ? 'javascript:void()' : PG_Blocks_v3::getLinkUrl( $args, 'link_url' ) ?>" class="link-secondary small stretched-link text-decoration-none"><?php echo PG_Blocks_v3::getAttribute( $args, 'link_label' ) ?></a>
        </div>
    </div>
</div>